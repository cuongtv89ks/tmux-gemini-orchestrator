...
nohup bash -c "sleep $SECONDS && tmux send-keys -t $TARGET 'Time for orchestrator check! cat next_check_note.txt && python3 tmux_utils.py' && sleep 1 && tmux send-keys -t $TARGET Enter" > /dev/null 2>&1 &
